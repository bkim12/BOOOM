function [O_opt, fval, comp_time] = BOOOM(objFun, O_initial, MaxTime, MaxRuns, MaxIter, sInitial, rho, TolFun1, TolFun2, phi, DisplayUpdate, DisplayEvery, PrintStepSize, PrintSolution)
...
for iii = 1:MaxRuns
    ...
    for i = 1:MaxIter
        ...
        for idx = 1:total_moves
            change_loc = ceil(idx / 2);  % Corresponds to the original rotation index
            sign = (-1)^(idx + 1);     % Alternates: +1 for odd idx, -1 for even idx
        
            pairs_i_idx = pairs_i(change_loc);
            pairs_j_idx = pairs_j(change_loc);
        
            O_rotated = create_premultiplied_matrix(O, pairs_i_idx, pairs_j_idx, sign * theta);

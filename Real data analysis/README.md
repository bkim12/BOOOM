...
In this study, $\lambda_1$ and  $\lambda_2$ take integer values **from 0 to 9**. To **keep the downstream scripts** (Pareto curve and optimal-solution analyses) aligned with the saved outputs, 
run the code by **fixing $\lambda_1$** and sweeping $\lambda_2$ from 0 to 9 in each run.
...
3. Run `pareto_curve.R` to read the results from Step 1 and plots the **Pareto curve**, highlighting penalty combinations that give the best trade-off between misclassification rate and sparsity.  

    **output**: pareto_curves.png

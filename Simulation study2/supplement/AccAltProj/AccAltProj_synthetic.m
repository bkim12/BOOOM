function [ L, S ] = AccAltProj_synthetic( D, r, para )
...
if  exist('./supplement/AccAltProj/PROPACK', 'dir')==7
    addpath('./supplement/AccAltProj/PROPACK');
    propack_exist = true;
else
    propack_exist = false;
...
% only changed tolereance beta and beta_init to synthetic data experiment
% settings.
% mu can be determined by get_mu_kappa.m algorithm but this require true L 
% So keep mu 5 as the author's setting/// gamma is also set as default in the algorithm. 
max_iter  = 100;
tol       = 1e-6;
gamma     = 0.5;    
mu        = 5;     
beta      = (1.1*mu)/(2*nthroot(m*n,2));
beta_init = (1.1*mu)/(nthroot(m*n,2));
trimming  = false;
